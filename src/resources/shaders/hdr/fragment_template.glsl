...
vec3 reinhardToneMapping(vec3 hdr_color, float gamma) {
    vec3 mapped = hdr_color / (hdr_color + vec3(1.0));
    mapped = pow(mapped, vec3(1.0 / gamma));
    return mapped;
}

vec3 filmicToneMapping(vec3 hdr_color, float gamma) {
...
vec3 uncharted2ToneMapping(vec3 hdr_color, float gamma, float exposure) {
    float A = 0.15;
    float B = 0.50;
    float C = 0.10;
    float D = 0.20;
...
vec3 acesToneMapping(vec3 hdr_color, float gamma) {
    const float a = 2.51;
    const float b = 0.03;
    const float c = 2.43;
    const float d = 0.59;
...
vec3 exponentialToneMapping(vec3 hdr_color, float gamma, float exposure) {
    vec3 mapped = vec3(1.0) - exp(-hdr_color * exposure);
    mapped = pow(mapped, vec3(1.0 / gamma));
    return mapped;
}

vec3 logarithmicToneMapping(vec3 hdr_color, float gamma) {
    vec3 mapped = log2(vec3(1.0) + hdr_color);
    mapped = pow(mapped, vec3(1.0 / gamma));
    return mapped;
}

vec3 mantiukToneMapping(vec3 hdr_color, float gamma) {
