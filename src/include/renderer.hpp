...
public:
    ...
    */
    void DrawSpriteSheet(Vec2 pos,Vec2 size,bool flipx,bool flipy,float row,float col,int layer,SpriteSheet &s);
    /**
    ...
    */
    void DrawAnimatedTexture(Vec2 pos,Vec2 size,int layer,bool reverse_x,bool reverse_y,SpriteSheet &s,bool &play_animation,bool loop_animation,float animation_delay,float &last_animation_time,int animation_row,int &animation_index);
    /**
     * Draw a triangle
...
private:
    ...
    inline int GetTexturesMinLayer(){ return m_Textures.NumVertices>0?m_BufferT[m_TextureIndex].layer:std::numeric_limits<int>::max(); }
    inline int GetPointsMinLayer(){ return m_Points.NumVertices>0?m_BufferP[m_PointIndex].layer:std::numeric_limits<int>::max(); }
    inline int GetLinesMinLayer(){ return m_Lines.NumVertices>0?m_BufferL[m_LineIndex].layer:std::numeric_limits<int>::max(); }
    inline int GetTrianglesMinLayer(){ return m_Triangles.NumVertices>0?m_BufferTR[m_TriangleIndex].layer:std::numeric_limits<int>::max(); }
